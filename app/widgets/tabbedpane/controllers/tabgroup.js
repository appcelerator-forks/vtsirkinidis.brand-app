...
function init() {
    ...
    tabInactiveColor = opts.tabInactiveColor || Alloy.CFG.colors.text_color;
...
function setUpTabStrip() {
    if (tabStripEnabled) {
        strip = Widget.createController('strip',{
            tabStripHeight : tabStripHeight,
            tabStripColor: tabStripColor
        }).getView();
        if (tabsOnTop) {
            strip.setTop(0);
        } else {
